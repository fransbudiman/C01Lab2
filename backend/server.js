...
app.get("/getAllNotes/:userId", express.json(), async (req, res) => {
  try {
    // Basic param checking
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title && !content) {
      return res
        .status(400)
        .json({ error: "Either one of title or content are required." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (data.matchedCount !=1) {
        return res
          .status(404)
          .json({ error: "Unable to find note with given ID belonging to " + decoded.username });
      }
